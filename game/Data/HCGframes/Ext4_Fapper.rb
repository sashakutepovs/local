...
p "Playing HCGframe : #{$game_player.actor.stat["EventTargetPart"]} FapperSlot4"
...
lona_mood "#{chcg_decider_basic_fapper}fuck_#{chcg_mood_decider}"
			#ext4
			if $game_player.actor.stat["pose"] == "chcg1" then  $game_player.actor.stat["chcg_x"] = -196 ; $game_player.actor.stat["chcg_y"] = -126 end
			if $game_player.actor.stat["pose"] == "chcg2" then  $game_player.actor.stat["chcg_x"] = -70 ; $game_player.actor.stat["chcg_y"] = -70 end
			if $game_player.actor.stat["pose"] == "chcg3" then  $game_player.actor.stat["chcg_x"] = -210 ; $game_player.actor.stat["chcg_y"] = -62 end
			if $game_player.actor.stat["pose"] == "chcg4" then  $game_player.actor.stat["chcg_x"] = -70 ; $game_player.actor.stat["chcg_y"] = -50 end
			if $game_player.actor.stat["pose"] == "chcg5" then  $game_player.actor.stat["chcg_x"] = -138 ; $game_player.actor.stat["chcg_y"] = -34 end
...
#stats_batch3
#stats_batch4
chcg_add_cums("EventExt4Race",["CumsMid","CumsHead","CumsBot","CumsTop"].sample)
#stats_batch6
check_over_event
#add cums body
#message control

...
$story_stats["OverMapEvent_name"] = "_bad_AbomManager"
$story_stats["OverMapEvent_enemy"] = 1
$story_stats["StepStoryCount"] = 0
sneak_result = overmap_event_sneak_check(70)
call_msg("OvermapEvents:World/ThingHappen_begin")
SndLib.sound_ManagerHurt(100)
chcg_background_color(0,0,0,255,-15) if @chcg_background_color
call_msg("OvermapEvents:World/ThingHappen_who_bad_AbomManager")
...
tmpOptions_Hide_able= true
	until loop_is_over ==1
		$story_stats["OverMapEvent_saw"] ==0 ? SndLib.sound_ManagerSpot(100) : SndLib.sound_ManagerAtk(100)
		...
		$game_player.actor.sta > 0 && $game_party.item_type_HowMany("Trap") >= 4 ? tmpOptions_Bomb = true : tmpOptions_Bomb = false
		tmpPicked = ""
		tmpQuestList = []
		...
		case tmpPicked
			when "Options_Escape"
					...
					running_result = overmap_event_running_check(50)
			...
			when "Options_Bomb"
					...
					$story_stats["OverMapEvent_saw"] =1
						...
						4.times{$game_party.decrease_item_type("Trap",1)}
						loop_is_over =1

...
call_msg("OvermapEvents:World/ThingHappen_who_unknow_AbandonedResources")
	...
	until loop_is_over ==1
		...
		case tmpPicked
			when "Options_Leave"
					...
					call_msg("OvermapEvents:Lona/Options_sneakpass_SneakWin") #不是敵人
					loop_is_over =1
					
			when "Options_Observe"
					tmpOptions_Observe_able = false
					sneak_result = overmap_event_sneak_check(0)
					$game_player.actor.sta -= 2
					$story_stats["HiddenOPT2"] = 0
					if $story_stats["OverMapEvent_enemy"] == 1
						call_msg("OvermapEvents:World/Check_Enemy_SneakWin")	if sneak_result == "_SneakWin"
						call_msg("OvermapEvents:World/Check_SneakFailed")		if sneak_result == "_SneakFailed"
					else
						call_msg("OvermapEvents:World/Check_NoEnemy_SneakWin")	if sneak_result == "_SneakWin"
						call_msg("OvermapEvents:World/Check_SneakFailed")		if sneak_result == "_SneakFailed"
					end
			...
			when "Options_Approach"
					if $story_stats["OverMapEvent_enemy"] ==1
						...
						return overmap_random_event_trigger(hidden_event,true,"AbandonedResources")
					else
						$story_stats["RegionMap_Background"] = "AbandonedResources"
						enter_tag_map = 1
						loop_is_over =1
					end

...
p "Playing HCGframe : #{$game_player.actor.stat["EventTargetPart"]} FapperSlot1"
...
lona_mood "#{chcg_decider_basic_fapper}fuck_#{chcg_mood_decider}"
			#ext1
			if $game_player.actor.stat["pose"] == "chcg1" then  $game_player.actor.stat["chcg_x"] = -218 ; $game_player.actor.stat["chcg_y"] = -178 end
			if $game_player.actor.stat["pose"] == "chcg2" then  $game_player.actor.stat["chcg_x"] = -81 ; $game_player.actor.stat["chcg_y"] = -59 end
			if $game_player.actor.stat["pose"] == "chcg3" then  $game_player.actor.stat["chcg_x"] = -274 ; $game_player.actor.stat["chcg_y"] = -56 end
			if $game_player.actor.stat["pose"] == "chcg4" then  $game_player.actor.stat["chcg_x"] = -85 ; $game_player.actor.stat["chcg_y"] = -72 end
			if $game_player.actor.stat["pose"] == "chcg5" then  $game_player.actor.stat["chcg_x"] = -165 ; $game_player.actor.stat["chcg_y"] = -64 end
...
#stats_batch3
#stats_batch4
chcg_add_cums("EventExt1Race",["CumsMid","CumsHead","CumsBot","CumsTop"].sample)
#stats_batch6
check_over_event
#add cums body
#message control

...
$game_player.actor.portrait.update
	$game_message.add("\\t[commonH:Lona/PeeonHead_choice_no]")
	$game_map.interpreter.wait_for_message
	##########################################################################################################
	if (10+$game_actors[1].wisdom)*rand(100) >= 50*rand(100)
		$game_message.add("\\t[commonH:Lona/PeeonHead_testWIN]")
		$game_map.interpreter.wait_for_message
	else
		$game_message.add("\\t[commonH:Lona/PeeonHead_testLOSE]")
		$game_map.interpreter.wait_for_message
		half_event_key_cleaner
		chcg_background_color_off
		load_script("Data/Batch/common_MCtorture_FunBeaten_event.rb")
		$game_message.add("\\t[commonH:Lona/PeeonHead_choice2]")
		$game_map.interpreter.wait_for_message
		if $game_temp.choice == 1
			$game_message.add("\\t[commonH:Lona/PeeonHead_choice_no]")
			$game_map.interpreter.wait_for_message
			if (20+$game_actors[1].wisdom)*rand(100) >= 50*rand(100)
				...
				load_script("Data/Batch/common_MCtorture_FunBeaten_event.rb")
				$game_message.add("\\t[commonH:Lona/PeeonHead_choice3]")
				$game_map.interpreter.wait_for_message
				if $game_temp.choice == 1
					$game_message.add("\\t[commonH:Lona/PeeonHead_choice_no]")
					$game_map.interpreter.wait_for_message
					if (20+$game_actors[1].wisdom)*rand(100) >= 50*rand(100)
						...
						$game_message.add("\\t[commonH:Lona/PeeonHead_testLOSE_end]")
						$game_map.interpreter.wait_for_message
					end
				end
			end

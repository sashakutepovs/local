...
tmpQuestList << [$game_text["commonComp:Companion/Disband"]			,"Disband"]
		...
		call_msg("CompDavidBorn:DavidBorn/CompCommand",0,2,0)
...
case tmpPicked
	when "Follow"
		...
		$game_map.popup(get_character(0).id,"CompDavidBorn:DavidBorn/CommandFollow#{rand(2)}",0,0)
		get_character(0).follower[1] =1

	when "Wait"
		SndLib.sound_QuickDialog 
		$game_map.popup(get_character(0).id,"CompDavidBorn:DavidBorn/CommandWait#{rand(2)}",0,0)

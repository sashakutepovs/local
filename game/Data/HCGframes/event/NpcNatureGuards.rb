...
if get_character(0).summon_data == nil
 ...
 get_character(0).summon_data[:SexTradeble] = true
	...
	call_msg("commonNPC:MaleHumanGuardM/begin1#{npc_talk_style}")
	...
	case tmpPicked
		when "Barter"
			manual_barters("NpcNatureGuardsWild")
		...
		when "Prostitution"
			...
			if $game_player.actor.sexy > temp_vs1
				...
				play_sex_service_menu(get_character(0),0.9)
		...
		when "Begging"
			1+rand(3).times{$game_player.actor.add_state("DoormatUp20")}
				...
				temp_tar = 150+rand(200) ##############   Begging Diff

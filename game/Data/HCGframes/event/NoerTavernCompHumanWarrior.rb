...
if get_character(0).summon_data == nil
 ...
 get_character(0).summon_data[:SexTradeble] = true
	...
	call_msg("commonComp:CompWarrior/begin")
	tmpGotoTar = ""
	tmpTarList = []
	...
	case tmpPicked
	when "TeamUp"
		call_msg("commonComp:CompWarrior/CompData")
		show_npc_info(get_character(0),extra_info=false,"commonComp:Companion/Accept") 			#\optD[算了，確定]
		case $game_temp.choice
			when 0,-1
			when 1
				if $game_player.player_slave?
					call_msg("commonComp:CompCommon/Slave_failed#{rand(3)}")
				elsif $game_party.item_number($data_items[56]) >=2 #啤酒2
					optain_lose_item($data_items[56],2)
					get_character(0).set_this_event_companion_front(get_character(0).name,false,$game_date.dateAmt+4)
					call_msg("commonComp:CompCommon/Comp_win")
				else
					call_msg("commonComp:CompCommon/Comp_failed")
				end
		end
	when "Disband"
		call_msg("commonComp:Companion/Accept") 			#\optD[算了，確定]
			case $game_temp.choice
				when 0,-1
				when 1
					call_msg("commonComp:CompCommon/Comp_disband")
					get_character(0).set_this_companion_disband(false)
			end
			
	when "Barter"
		manual_barters("NoerTavernCompCommon")

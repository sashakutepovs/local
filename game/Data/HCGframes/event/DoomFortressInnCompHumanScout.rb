...
if get_character(0).summon_data == nil
 ...
 get_character(0).summon_data[:SexTradeble] = true
	...
	call_msg("commonComp:CompHumanScout/begin")
	tmpGotoTar = ""
	tmpTarList = []
	...
	case tmpPicked
	when "TeamUp"
		call_msg("commonComp:CompHumanScout/CompData")
		...
		case $game_temp.choice
			...
			when 1
				if $game_player.actor.stat["SlaveBrand"] != 0
					call_msg("commonComp:CompHumanScout/Comp_failed_slave")
					...
					call_msg("commonComp:CompHumanScout/Comp_win")
				else
					call_msg("commonComp:CompHumanScout/Comp_failed")
				end
		end
	when "Disband"
		call_msg("commonComp:Companion/Accept") 			#\optD[算了，確定]
			case $game_temp.choice
				when 0,-1
				when 1
					call_msg("commonComp:CompHumanScout/Comp_disband")
					get_character(0).set_this_companion_disband(false)
			end
			
	when "Barter"
		manual_barters("DoomFortressInnCompHumanScout")

...
if $game_player.actor.record_lona_title != "WhoreJob/SaintBeliever"
	...
	call_msg_popup("TagMapSaintMonastery:patient/Qmsg1",get_character(0).id)
	return
end
if get_character(0).summon_data == nil
 ...
 get_character(0).summon_data[:SexTradeble] = true
	...
	call_msg("TagMapSaintMonastery:MaleBeliever/guard")
	...
	case tmpPicked
		when "Barter"
			manual_barters("SaintMonasteryGuardM")
		...
		when "Begging"
			1+rand(3).times{$game_player.actor.add_state("DoormatUp20")}
				...
				temp_tar = 100+rand(250) ##############   Begging Diff

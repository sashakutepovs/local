...
if tmp_fucker_id == nil
	...
	############################################################################# Normal RapeLoop day Pass
	if $game_player.actor.stat["SlaveBrand"] != 0 && $story_stats["SlaveOwner"] == "NorthFL_INN" && $story_stats["Captured"] == 1
		tmpDualBiosID = $game_map.get_storypoint("DualBios")[2]
		
		if get_character(tmpDualBiosID).summon_data[:Qtaken] == true && get_character(tmpDualBiosID).summon_data[:JobDone] == false && get_character(tmpDualBiosID).summon_data[:IsSlave] == true
			$story_stats["RapeLoopTorture"] = 1
			get_character(tmpDualBiosID).summon_data[:Qtaken] = false
			get_character(tmpDualBiosID).summon_data[:JobDone] = false
		elsif get_character(tmpDualBiosID).summon_data[:Qtaken] == true || get_character(tmpDualBiosID).summon_data[:JobDone] == true && get_character(tmpDualBiosID).summon_data[:IsSlave] == true
			get_character(tmpDualBiosID).summon_data[:Qtaken] = false
			get_character(tmpDualBiosID).summon_data[:JobDone] = false
		end
		if $story_stats["RapeLoopTorture"] == 1
			chcg_background_color(0,0,0,0,7)
			call_msg("TagMapNorthFL_INN:Torture/NapTrigger")
	...
	elsif $story_stats["Captured"] == 0 && $game_player.player_slave? && $game_player.actor.stat["SlaveBrand"] != 0
		### todo capture
	...
	elsif $game_player.actor.morality < 1 || $game_player.player_slave? || $game_player.actor.stat["SlaveBrand"] != 0
		$story_stats["OnRegionMapSpawnRace"] = "HumanGuard"
		return handleNap if !check_enemy_survival?("HumanGuard",false)
		chcg_background_color(0,0,0,0,7)
		call_msg("TagMapPirateBane:Guard/NapLowMora")
		$story_stats["RapeLoop"] = 1
		...
		$game_map.npcs.each{|event|
				...
				event.summon_data[:WildnessNapEvent] = "BanditMobs"
		}
		$story_stats["Kidnapping"] =1
		$game_player.actor.sta =-100
		region_map_wildness_nap
	
	elsif !$game_party.has_item?($data_items[129]) #ItemNorthFL_INN_Key
		call_msg("common:Lona/NarrDriveAway")
		change_map_leave_tag_map
		portrait_hide
		
	elsif $game_party.has_item?($data_items[129]) #ItemNorthFL_INN_Key
		$game_party.lose_item($data_items[129], 1) #ItemNorthFL_INN_Key

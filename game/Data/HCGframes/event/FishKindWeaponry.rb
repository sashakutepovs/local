...
return call_msg("TagMapFishTown:Trader/TooWeak") if $game_player.actor.weak >= 25
	call_msg("TagMapFishTown:WeaponTrader/Welcome#{rand(2)}")
	...
	tmp_Begging = false#$game_player.actor.stat["SlaveBrand"] == 1 && $game_player.actor.weak >= 50 && get_character(0).summon_data[:SexTradeble]
	tmpGotoTar = ""
	tmpTarList = []
	...
	case tmpPicked
		when "Barter"
			manual_barters("FishKindWeaponry")

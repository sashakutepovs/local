...
if $story_stats["OverMapEvent_name"] != 0 || !get_character(0).summon_data || get_character(0).summon_data[:executed]
	...
	call_msg_popup("TagMapAbanHouse2:EmptyHouse/Qmsg1")
	return
end
...
call_msg("commonNPC:NoerHouse/Tent")
...
$game_temp.choice = -1
	...
	case tmpPicked
		when "Begging"
				...
				temp_tar = 50+rand(300) ##############   Begging Diff
				...
				if tmpWeak >=  temp_tar #### WIN
					...
					call_msg("commonNPC:NoerHouse/Tent_beg_fail0")
					tmpSumTar=["NeutralHumanHoboRageM","NeutralHumanHoboRageF","NeutralMootHoboRageM","NeutralMootHoboRageF"].sample
					EvLib.sum(tmpSumTar,$game_player.x,$game_player.y) if rand(100) >=50
				end
				$game_player.actor.update_lonaStat
		when "Deceive"
				call_msg("commonNPC:NoerHouse/TentOpt_lie_begin1")
				...
				$game_player.actor.sta -= 3
				temp_tar= 15+rand(80)
				temp_vs = $game_player.actor.wisdom + rand(100)
				call_msg("\\narr #{temp_vs.round} VS #{temp_tar.round}")
				if temp_vs >= temp_tar #win
					call_msg("commonNPC:NoerHouse/HouseOpt_lie_win")
					itemList = [48,14,145].sample #roach,rat,ItemDreg
					$game_party.gain_item($data_items[itemList], 1)
					$game_map.popup(0,:item,itemList,1)
					SndLib.sys_Gain
				else #fail
		...
		when "Steal"
				...
				if temp_vs >= temp_tar #win
					...
					item = $data_items[145]			;	$game_boxes.box(box_id)[item] = rand(3)+1 #ItemDreg
					item = $data_items[48]			;	$game_boxes.box(box_id)[item] = 1 if rand(100) >=50 #AnimalRoach
					item = $data_items[14]			;	$game_boxes.box(box_id)[item] = 1 if rand(100) >=50 #AnimalRattus
					item = $data_items[28]			;	$game_boxes.box(box_id)[item] = 1 if rand(100) >=90 #ItemHerbContraceptive
					item = $data_items[26]			;	$game_boxes.box(box_id)[item] = 1 if rand(100) >=90 #ItemHerbCure
					item = $data_items[27]			;	$game_boxes.box(box_id)[item] = 1 if rand(100) >=90 #ItemHerbSta
					...
					tmpSumTar = ["NeutralHumanHoboRageM","NeutralHumanHoboRageF","NeutralMootHoboRageM","NeutralMootHoboRageF"].sample
					EvLib.sum(tmpSumTar,$game_player.x,$game_player.y)
				end
				$game_player.actor.update_lonaStat
	end #case

...
if get_character(0).summon_data == nil
	...
	call_msg("commonComp:CompHumanExplorer/begin")
	tmpGotoTar = ""
	tmpTarList = []
	...
	case tmpPicked
	when "TeamUp"
		call_msg("commonComp:CompHumanExplorer/CompData")
		...
		case $game_temp.choice
			...
			when 1
				if $game_party.item_number($data_items[42]) >=1 #肉湯
					...
					get_character(0).set_this_event_companion_back(get_character(0).name,false,$game_date.dateAmt+5)
					call_msg("commonComp:CompHumanExplorer/Comp_win")
				else
					call_msg("commonComp:CompHumanExplorer/Comp_failed")
				end
		end
	when "Disband"
		call_msg("commonComp:Companion/Accept") 			#\optD[算了，確定]
			case $game_temp.choice
				when 0,-1
				when 1
					call_msg("commonComp:CompHumanExplorer/Comp_disband")

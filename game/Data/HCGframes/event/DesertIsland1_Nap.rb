...
tmpUndeadAlive = $game_map.npcs.any?{|ev|
	next if !ev.summon_data
	next if !ev.summon_data[:WildnessNapEvent] == "UndeadWalking"
	next if ev.npc.action_state == :death
	next if ev.deleted? || ev.erased
	true
}
if tmpUndeadAlive && $story_stats["OnRegionMapSpawnRace"] == "UndeadWalking"
	$story_stats["RapeLoop"] = 1
	$story_stats["OnRegionMapSpawnRace"] = "UndeadWalking"
	return region_map_wildness_nap
elsif $story_stats["OnRegionMapSpawnRace"] == "UndeadWalking"
	return handleNap
end
####################################################
tmpRaperAlive = $game_map.npcs.any?{|ev|
	next if !ev.summon_data
	next if !ev.summon_data[:Commoner]
	next if ev.region_id == 54
	next if ev.npc.action_state == :death
	next if ev.deleted? || ev.erased
	true
}

if !tmpRaperAlive || get_character(tmpDualBiosID).summon_data[:AllDead] == true
	get_character(tmpDualBiosID).summon_data[:AllDead] = true
	$game_map.events.each{|event|
		next unless event[1].summon_data
		next unless event[1].summon_data[:Flower]
		next unless !event[1].deleted? || !event[1].erased
		event[1].nap_initialize = 3
		event[1].reset_strong = false
	}
	return handleNap
end
do_ret = false
...
until do_ret
	...
	if tmp_fucker_id != nil && !$game_map.threat
		...
		call_msg("TagMapDesertIsland1:NapRape/0_#{rand(2)}")
		...
		get_character(tmp_fucker_id).turn_toward_character($game_player)
		call_msg("TagMapDesertIsland1:NapRape/1")
		...
		call_msg("TagMapDesertIsland1:NapRape/2")
		...
		if $game_player.actor.sta > 0
			...
			case $game_temp.choice
				...
				when 1
					player_cancel_nap
					do_ret = true
					get_character(tmp_fucker_id).actor.set_aggro($game_player.actor,$data_arpgskills["BasicNormal"],600)
					$game_player.turn_toward_character(get_character(tmp_fucker_id))
					...
					get_character(tmp_fucker_id).call_balloon(20)
					call_msg("TagMapDesertIsland1:NapRape/angry")
			end
			$game_temp.choice = -1

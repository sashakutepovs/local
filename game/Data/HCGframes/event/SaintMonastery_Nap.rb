...
if tmp_fucker_id == nil
	#cocona quest failed
	if $story_stats["RecQuestCocona"].between?(16,20) || $game_player.actor.morality < 0 || $story_stats["RecQuestSMRefugeeCampFindChild"] == 8
		...
		call_msg("TagMapSaintMonastery:Nap/Capture")
		$story_stats["RapeLoop"] = 1
		$story_stats["RapeLoopTorture"] =1
		$story_stats["Captured"] = 1
		change_map_captured_enter_tag(target="SaintMonasteryB1")

	#rapeloop capture or slave captured
	elsif $story_stats["RapeLoop"] == 1 && !get_character(tmpDualBiosID).summon_data[:Captured]
		$story_stats["RapeLoopTorture"] = 1
		get_character(tmpDualBiosID).summon_data[:Captured] = true
		call_msg("TagMapSaintMonastery:Nap/Capture")
		rape_loop_drop_item(false,false)

...
if get_character(0).summon_data == nil
 ...
 get_character(0).summon_data[:SexTradeble] = true
	...
	call_msg("commonComp:CompFishShaman/begin")
	...
	tmpTarList << [$game_text["commonComp:Companion/TeamUp"]			,"TeamUp"] if canFuck && $game_player.record_companion_name_back != get_character(0).name && !$game_player.player_slave?
	...
	case tmpPicked
	when "TeamUp"
		call_msg("commonComp:CompFishShaman/CompData")
		call_msg("commonComp:CompFishShaman/sex0")
		...
		case $game_temp.choice
			...
			when 1
				...
				$cg.erase
				call_msg("commonComp:CompFishShaman/sex1")
				...
				call_msg("commonComp:CompFishShaman/Comp_win")
					...
					call_msg("commonComp:CompFishShaman/Comp_failed_slave")
				else
					call_msg("commonComp:Lona/notWhore0")
					call_msg("commonComp:CompFishShaman/Comp_failed")
	...
	when "Disband"
		call_msg("commonComp:Companion/Accept") 			#\optD[算了，確定]
			case $game_temp.choice
				when 0,-1
				when 1
					call_msg("commonComp:CompFishShaman/Comp_disband")
					get_character(0).set_this_companion_disband(false)
			end
			
	when "Barter"
			manual_barters("FishTownInnCompFishShaman")
	when "Prostitution"
		get_character(0).summon_data[:SexTradeble] = false
		$game_temp.choice == 0
		call_msg("commonNPC:RandomNpc/WhoreWork#{talk_style}")

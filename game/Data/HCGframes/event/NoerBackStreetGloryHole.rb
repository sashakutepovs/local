...
if !get_character(0).summon_data[:tmpPenisID]
	get_character(0).summon_data[:tmpPenisID] =  ["Normal","Hairly","Smegma","Mega"].sample
end
tmpPenisID = get_character(0).summon_data[:tmpPenisID]
...
case rand(2)
	...
	#$game_player.actor.add_state("STD_WartMouth") if [true,false,false].sample
end
...
if tmpFailed
	...
	$game_portraits.setLprt("nil")
	call_msg("TagMapNoerBackStreet:Lona/WorkFailed")
	if tmpWorkMode
		$story_stats["BackStreetArrearsPrincipal"] += 2000
		call_msg("\\narr #{$game_text['commonCommands:Lona/LoanPrincipal']}+2000")
	elsif $story_stats["RapeLoop"] == 1 || $story_stats["Captured"] == 1
		$story_stats["RapeLoopTorture"] = 1
	end
end
#############################################################QA END
play_sex_service_items(nil,["ItemCoin1","ItemCoin2","ItemCoin3"],prev_gold)

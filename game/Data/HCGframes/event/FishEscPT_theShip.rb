...
get_character(0).summon_data[:maxLvlRec] =  get_character(0).summon_data[:buildLVL] if !get_character(0).summon_data[:maxLvlRec]
if tmpBuildLVL > 0
	...
	optain_lose_item("ItemMhWoodenClub",$game_temp.choice)
		...
		elsif tmpBuildLVL < (get_character(0).summon_data[:maxLvlRec]/2).round

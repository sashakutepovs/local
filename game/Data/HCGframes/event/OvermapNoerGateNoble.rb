...
portrait_off
call_msg("OvermapNoer:GateNobGuard/begin1")
...
case $game_temp.choice
	...
	when 1 #通行證
			...
			if $game_player.direction == 2
				if $game_party.has_item_type("NoerPassportNob") && $game_party.has_item_type("NoerPassport")
					if $game_player.actor.morality_lona >=30 && $game_player.actor.stat["SlaveBrand"] == 0
						call_msg("OvermapNoer:GateGuard/enter_passport_NoDir")
				...
				if !($game_player.actor.morality_lona >=30 && $game_player.actor.stat["SlaveBrand"] == 0)
					...
					call_msg("OvermapNoer:GateGuard/enter_failed#{talk_persona}")
					load_script("Data/HCGframes/encounter/NoerGuards.rb")
				elsif $game_player.actor.dirt >= 125
					call_msg("OvermapNoer:GateGuard/enter_failedDirt1")
					call_msg("OvermapNoer:GateGuardOuta/enter_failed#{talk_persona}")
					call_msg("OvermapNoer:GateGuard/enter_failedDirt2")
					
				elsif $game_party.has_item_type("NoerPassportNob") && $game_party.has_item_type("NoerPassport")
					call_msg("OvermapNoer:GateGuard/enter_passport_NoDir")
					$game_player.jump_forward(2)

...
tmpAllGuardDid = $game_map.npcs.any? {|event|
	next if !event.summon_data
	next if !event.summon_data[:prostituting]
	next if event.switch1_id == 1
	true
}
...
if !tmpAllGuardDid
	...
	$game_map.npcs.each{|event|
		...
		event.npc.fucker_condition={"sexy"=>[60, ">"]}
		event.npc.sense_target($game_player,0)
		event.set_manual_move_type(1)
		event.move_type = 1 if event.move_type == 0
		event.call_balloon(4)
	}
	call_msg("TagMapNoerEastCp:whoreWork/FuckStart")
end

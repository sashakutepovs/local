...
tmpNurseAlive = $game_map.npcs.any?{|ev|
	...
	call_msg("TagMapSouthFL:GuardDoomed/beginRNG#{rand(3)}")
	...
	case tmpPicked
		when "Purge"
			...
			get_character(nurseEvID).turn_toward_character($game_player)
			call_msg("TagMapSouthFL:healerFInn/WorkDOing_begin0Guard_#{rand(3)}")
			...
			call_msg("TagMapSouthFL:healerFInn/WorkDOing_begin4Guard")
			$cg.erase
			whole_event_end
		...
		when "Barter"
			manual_barters("SouthFLGuardsDoomed")
		...
		when "Prostitution"
			...
			call_msg("\\narr #{$game_player.actor.sexy.round} VS #{temp_vs1.round}")
				if $game_player.actor.sexy > temp_vs1
					...
					prev_gold = $game_party.gold
					tmpSuccess = play_sex_service_menu(get_character(0),0.6,"closest")
					if !["Repent","Break"].include?(tmpSuccess)
						play_sex_service_items(get_character(0),["ItemCoin1","ItemCoin2","ItemCoin3"],prev_gold,summon=true)
					end
				else
		...
		when "Begging"
			1+rand(3).times{$game_player.actor.add_state("DoormatUp20")}
				...
				temp_tar = 300+rand(650) ##############   Begging Diff

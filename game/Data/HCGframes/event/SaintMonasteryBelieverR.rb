...
################################################################ 尋找湯米的任務
if $story_stats["RecQuestSMRefugeeCampFindChild"] == 1 && get_character(0).summon_data[:QuprogTommy] == 1
	get_character(0).call_balloon(0)
	$story_stats["RecQuestSMRefugeeCampFindChild"] = 2
	call_msg("TagMapSaintMonastery:MIAchildMAMA/prog1")
	$game_map.npcs.any?{|event|
		next if event.summon_data == nil
		next unless event.summon_data[:QuprogTommy] == 2
		next if event.actor.action_state == :death
		event.call_balloon(28,-1)
	}
	return eventPlayEnd
################################################################ 以下為基本款
elsif $game_player.actor.record_lona_title != "WhoreJob/SaintBeliever"
	SndLib.sound_QuickDialog
	call_msg_popup("TagMapSaintMonastery:follower/Qmsg#{rand(4)}",get_character(0).id)
	return
end
if get_character(0).summon_data == nil
 ...
 get_character(0).summon_data[:SexTradeble] = true
	...
	call_msg("TagMapSaintMonastery:MaleBeliever/Prayer_begin#{rand(3)}")
	...
	case tmpPicked
		when "Barter"
			manual_barters("SaintMonasteryBelieverM")
			
		when "Prostitution"
			get_character(0).summon_data[:SexTradeble] = false
			$game_temp.choice == 0
			call_msg("TagMapSaintMonastery:Lona/WhoreJobStart0")
			call_msg("TagMapSaintMonastery:MaleBeliever/PrayerWhoreJobStart1")

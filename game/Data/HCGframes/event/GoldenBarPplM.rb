...
if $game_player.actor.dirt >= 150
	...
	call_msg_popup("TagMapNoerGoldenBar:Commoner/BeginDirt#{rand(3)}",get_character(0).id)
	return
end
...
if get_character(0).summon_data == nil
 ...
 get_character(0).summon_data[:SexTradeble] = true
	...
	call_msg("TagMapNoerGoldenBar:Commoner/Begin#{rand(3)}")
	...
	case tmpPicked
		...
		when "Prostitution"
			...
			if $game_player.actor.sexy > temp_vs1
				...
				play_sex_service_menu(get_character(0),1,"rand")
		...
		when "Begging"
			1+rand(3).times{$game_player.actor.add_state("DoormatUp20")}
				...
				tmpWeak = $game_player.actor.weak - $game_player.actor.stat["SlaveBrand"]*rand(10) - $game_player.actor.stat["moot"]*rand(10) + $game_player.actor.wisdom*rand(10)

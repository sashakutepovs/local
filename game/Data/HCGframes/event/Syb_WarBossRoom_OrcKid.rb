...
if !get_character(0).summon_data[:WarbossTalked]
	get_character(0).npc.play_sound(:sound_alert1,80,130)
	get_character(0).call_balloon(8)
	wait(60)
	get_character(0).npc.play_sound(:sound_alert2,80,130)
	get_character(0).jump_to(get_character(0).x,get_character(0).y)
	get_character(0).call_balloon(20)
	wait(60)
	call_msg("TagMapSyb_WarBossRoom:OrcKid/begin1")
	...
	get_character(0).npc.play_sound(:sound_alert2,80,180)
	wait(60)
	call_msg("TagMapSyb_WarBossRoom:OrcKid/Talked1")
end
...
case tmpPicked
	...
	when "rapeloop_OPT_rape"
		...
		npc_sex_service_main($game_player.actor.master,get_character(0),"anal",nil,1)
		10.times{
			SndLib.sound_chs_buchu(80)
			get_character(0).npc.play_sound(:sound_death,80,130) if rand(100) >= 60
			wait(30)
		}
		get_character(0).summon_data[:WarbossTalked] = true

...
$game_player.actor.scoutcraft_trait >=10	? $story_stats["HiddenOPT1"] = "1" : $story_stats["HiddenOPT1"] = "0"
$game_player.actor.wisdom_trait >=10		? $story_stats["HiddenOPT2"] = "1" : $story_stats["HiddenOPT2"] = "0"


portrait_off
call_msg("TagMapDoomFortress:ThisMap/OvermapEnter")
...
case $game_temp.choice
	...
	when 1 #通行證
			call_msg("OvermapNoer:GateGuard/enter_wait")
				if $game_player.record_companion_name_back == "UniqueCecily"
					...
					change_map_enter_tag("DoomFortressL")
					
				elsif $game_party.has_item_type("NoerPassport")
					if $game_player.actor.morality_lona >=1 && $game_player.actor.stat["SlaveBrand"] == 0
						call_msg("OvermapNoer:GateGuard/enter_with_passport")
						$game_player.direction = 6
						change_map_enter_tag("DoomFortressL")
					else
						call_msg("OvermapNoer:GateGuardOuta/enter_LowMorality")
						call_msg("OvermapNoer:GateGuardOuta/enter_failed#{talk_persona}")
					end
				else #沒票
					if $game_player.actor.morality_lona >=1 && $game_player.actor.stat["SlaveBrand"] == 0
	...
	when 2 #隱匿
			...
			if temp_roll_skill >= temp_roll_diff #隱匿成功
				...
				if $game_player.actor.morality_lona >=1 && $game_player.actor.stat["SlaveBrand"] == 0 #失敗 扣MOR

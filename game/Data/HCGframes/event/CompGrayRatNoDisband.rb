...
if $story_stats["QuProgSaveCecily"] < 4
	...
	tmpQuestList << [$game_text["commonComp:Companion/Wait"]			,"Wait"]
			...
			call_msg("CompGrayRat:GrayRat/Comp_start",0,2,0)
	...
	case tmpPicked
		when "Follow"
			...
			$game_map.popup(get_character(0).id,"CompGrayRat:GrayRat/CommandFollow#{rand(2)}",0,0)
			get_character(0).follower[1] =1
	
		when "Wait"
			SndLib.sound_QuickDialog 
			$game_map.popup(get_character(0).id,"CompGrayRat:GrayRat/CommandWait#{rand(2)}",0,0)
			get_character(0).follower[1] =0
			
		when "Disband"
			call_msg("CompGrayRat:GrayRat/Comp_disband")

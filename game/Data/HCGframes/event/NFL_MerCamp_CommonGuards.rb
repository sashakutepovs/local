...
if $story_stats["UniqueChar_NFL_MerCamp_Leader"] == -1
	...
	get_character(0).set_summon_data({:SexTradeble => true}) if get_character(0).summon_data == nil
	get_character(0).summon_data[:SexTradeble] = true if get_character(0).summon_data[:SexTradeble] == nil
			call_msg("TagMapNFL_MerCamp:commoner/BeginRNG#{rand(3)}")
		...
		case tmpPicked
			when "Barter"
			 manual_barters("NFL_MerCamp_CommonGuards")
			...
			when "Prostitution"
				...
				if $game_player.actor.weak > temp_vs1
					...
					play_sex_service_menu(get_character(0),0.8)
					tmpReward = [
						"ItemHumanoidFlesh",	"ItemBread",
						"ItemHerbCure",			"ItemMushroom",
						"ItemMutantFlesh",		"ItemHerbSta",
						"ItemDryFood"
						]
					play_sex_service_items(get_character(0),tmpReward.sample(3),prev_gold)
				else
					$game_player.actor.mood -=3
					call_msg("TagMapNFL_MerCamp:commoner/WhoreWork_failed#{rand(3)}")
				end

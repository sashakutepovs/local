...
tmpMobAlive = $game_map.npcs.any?{|event| 
	...
	next if event.summon_data[:ignoreWhenNap]
	next if event.deleted?
	next if event.npc.action_state == :death
	true
}
#atkBossDead = $story_stats["UniqueCharSMCloudVillage_BossAtk"] == -1
#leaderBossDead = $story_stats["UniqueCharSMCloudVillage_Boss"] == -1
SndLib.bgm_stop
...
if (tmpG1HP >= tmpG1HPmax || tmpG2HP >= tmpG2HPmax) && tmpMobAlive
	rape_loop_drop_item(false,false)
	$story_stats["RapeLoopTorture"] = 1 if $story_stats["Captured"] == 1
...
if $story_stats["RapeLoopTorture"] == 1 && tmpMobAlive
	...
	call_msg("TagMapSMCloudVillage:Common/NapTorture1")
	cam_center(0)
	...
	call_msg("TagMapSMCloudVillage:Common/NapTorture2")
	cam_center(0)
	wait(60)
	...
	get_character(tmpFucker1ID).animation = get_character(tmpFucker1ID).animation_atk_sh
	wait(60)
	call_msg("TagMapSMCloudVillage:Common/NapTorture3")
	cam_center(0)
...
$game_map.npcs.each do |event| 
	next unless event.summon_data
	next unless event.summon_data[:WildnessNapEvent] == "Villager"
...
until do_ret
	...
	if tmp_fucker_id != nil
		...
		get_character(tmp_fucker_id).turn_toward_character($game_player)
		call_msg("TagMapSMCloudVillage:Common/NapRape1")
		...
		call_msg("TagMapSMCloudVillage:Common/NapRape3_#{rand(3)}")
		if $game_player.actor.sta > 0
			...
			case $game_temp.choice
				when 0
					...
					call_msg("TagMapSMCloudVillage:Common/NapRape_withSta_NoFight")
				when 1
					player_cancel_nap
					do_ret = true
					...
					get_character(tmp_fucker_id).call_balloon(20)
					call_msg("TagMapSMCloudVillage:Common/NapRape_withSta_Fight")
			end
			$game_temp.choice = -1
...
if tmpMobAlive && $story_stats["Captured"] == 0
	...
	call_msg("TagMapSMCloudVillage:alert/Captured") if $story_stats["Captured"] == 0
	rape_loop_drop_item(false,false)
	#$story_stats["RapeLoopTorture"] = 1 if atkBossDead && leaderBossDead #若大哥死了則處刑 

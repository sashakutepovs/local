...
call_msg("TagMapNoerBackStreet:HappyTrader/begin_normal") if $story_stats["BackStreetArrearsPrincipal"] == 0
call_msg("TagMapNoerBackStreet:HappyTrader/begin_normalArrears") if $story_stats["BackStreetArrearsPrincipal"] > 0
...
case tmpPicked
	when "Barter"
			manual_barters("NoerBackStreetHappyTrader")
	when "Work"
			tmpGotoTar = ""
			tmpTarList = []
			...
			call_msg("TagMapNoerBackStreet:HappyTrader/work_opt",0,2,0)
			call_msg("\\optB[#{cmd_text}]")
			$game_temp.choice == -1 ? tmpPicked = false : tmpPicked = cmd_sheet[$game_temp.choice][1]
			$game_temp.choice = -1
			case tmpPicked
					when "Employee"
						call_msg("TagMapNoerBackStreet:HappyTrader/BecomeSlave_begin0")
						call_msg("TagMapNoerBackStreet:HappyTrader/BecomeSlave_begin1")
						...
						if $game_temp.choice == 1
							...
							call_msg("TagMapNoerBackStreet:HappyTrader/BecomeSlave_accept1")
						...
						if $game_player.actor.stat["Prostitute"] == 1
							...
							case $game_temp.choice
								when 1
									...
									call_msg("TagMapNoerBackStreet:HappyTrader/work_whore_accept0")
									call_msg("TagMapNoerBackStreet:HappyTrader/work_whore_accept1") if $story_stats["UniqueCharUniqueGangBoss"] != -1
									cam_center(0)
							end
					else
						call_msg("TagMapNoerBackStreet:HappyTrader/work_whore_NoTrait")
					end
				end
	when "LoanInfo"
			call_msg("TagMapNoerBackStreet:HappyTrader/opt_AboutArrears")
	...
	when "Loan"
			...
			if $story_stats["RecordBackStreetArrears"] == 0
				...
				call_msg("TagMapNoerBackStreet:HappyTrader/Arrears_NewTo0")
				call_msg("TagMapNoerBackStreet:HappyTrader/Arrears_NewTo1")
				call_msg("TagMapNoerBackStreet:HappyTrader/Arrears_NewTo2")
			end
			call_msg("TagMapNoerBackStreet:HappyTrader/Arrears1_opt") # [算了,20000,40000,60000,80000]
			case $game_temp.choice
				when 0,-1
				else
					...
					call_msg("TagMapNoerBackStreet:HappyTrader/Arrears_accept")
			end
		when "Repayment"
			tmpGotoTar = ""
			tmpTarList = []
			...
			call_msg("TagMapNoerBackStreet:HappyTrader/Arrears_return_opt",0,2,0)
			call_msg("\\optB[#{cmd_text}]")
			$game_temp.choice == -1 ? tmpPicked = false : tmpPicked = cmd_sheet[$game_temp.choice][1]
			$game_temp.choice = -1
			...
			case tmpPicked
				...
				when "PawnItems"
						...
						call_msg("TagMapNoerBackStreet:HappyTrader/Arrears_return_item")

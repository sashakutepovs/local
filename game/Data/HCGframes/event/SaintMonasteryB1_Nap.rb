...
get_character(tmpG2ID).switch1_id = 0 if get_character(tmpG2ID).switch1_id == nil
tmpG2HP = get_character(tmpG2ID).switch1_id
tmpG2HPmax = get_character(tmpG2ID).summon_data[:staNeed]
if tmpG2HP >= tmpG2HPmax
...
if ($story_stats["RapeLoop"] == 1 || $story_stats["RapeLoopTorture"] == 1) && $story_stats["Captured"] == 0
...
if tmpMobAlive
	 if $story_stats["Captured"] == 0
		tmpG2X,tmpG2Y,tmpG2ID = $game_map.get_storypoint("Gate2")
		get_character(tmpG2ID).switch1_id = 0
		call_msg("TagMapSaintMonasteryB1:belever/NapSpot")
	end
	rape_loop_drop_item(false,false)
	$story_stats["Captured"] = 1
...
if (tmpDoRape ==1 && rand(100) >= 60) || $story_stats["RapeLoopTorture"] ==1
	...
	chcg_background_color(0,0,0,255)
		...
		get_character(tmpFucker2ID).moveto($game_player.x+1,$game_player.y)	if !get_character(tmpFucker2ID).nil?
		...
		call_msg("TagMapSaintMonasteryB1:belever/NapRape") if $story_stats["RapeLoopTorture"] ==0
		call_msg("TagMapSaintMonasteryB1:belever/NapTorture1") if $story_stats["RapeLoopTorture"] ==1
	...
	if $story_stats["RapeLoopTorture"] ==1
		...
		call_msg("TagMapSaintMonasteryB1:belever/NapTorture2")
		wait(60)
		...
		get_character(tmpFucker1ID).animation = get_character(tmpFucker1ID).animation_atk_sh
		wait(60)
		call_msg("TagMapSaintMonasteryB1:belever/NapTorture3")
...
until do_ret
	...
	if tmp_fucker_id != nil
		...
		call_msg("TagMapSaintMonasteryB1:belever/NapRape_#{rand(3)}")
		...
		get_character(tmp_fucker_id).turn_toward_character($game_player)
		call_msg("TagMapSaintMonasteryB1:belever/NapRape1")
		...
		call_msg("TagMapSaintMonasteryB1:belever/NapRape3_#{rand(3)}")
		if $game_player.actor.sta > 0
			...
			case $game_temp.choice
				when 0
					...
					call_msg("TagMapSaintMonasteryB1:belever/NapRape_withSta_NoFight2")
				when 1
					player_cancel_nap
					do_ret = true

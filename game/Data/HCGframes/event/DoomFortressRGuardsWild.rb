...
####################################### SLAVE ###############################
if $story_stats["Captured"] == 1 && get_character(0).summon_data[:NightPatrol] == true
	...
	call_msg_popup("TagMapDoomFortress:NightPatrol/SlaveQmsg#{rand(3)}",get_character(0).id)

####################################### Escape Slave ###############################
elsif $story_stats["Captured"] == 0 && $game_player.actor.stat["SlaveBrand"] == 1
...
####################################### NORMAL ###############################
	...
	case tmpPicked
		when "Barter"
			manual_barters("DoomFortressGuardsWild")
		...
		when "Prostitution"
			...
			if $game_player.actor.sexy > temp_vs1
				...
				play_sex_service_menu(get_character(0),0.7,"closest")

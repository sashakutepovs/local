...
class Scene_File < Scene_MenuBase
  ...
  def create_help_window
	...
	#@help_window.set_text($game_text["menu:SaveLoad/Top"])
  ...
  #--------------------------------------------------------------------------
	def create_savefile_viewport
		...
		@savefile_viewport.rect.y = @help_window.height/2
		@savefile_viewport.rect.height -= @help_window.height
		@savefile_viewport.z= System_Settings::SCENE_BASE_Z
	end
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
	...
	def create_saveoverview_window
		@overview_window = Window_SaveOverview.new(Graphics.width/2,@savefile_viewport.rect.y,Graphics.width/2-32,savefile_height*4)
	end
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
	def item_max
		DataManager.saveFileMAX - 2 #-2 for doom and auto
	end
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
	def update_savefile_selection
		return on_savefile_ok     if Input.trigger?(:C) || WolfPad.trigger?(:Z_LINK)
		return on_savefile_cancel if Input.trigger?(:B) || WolfPad.trigger?(:X_LINK)
		update_cursor
	end
  #--------------------------------------------------------------------------
  # * Save File [OK]
  ...
  #--------------------------------------------------------------------------
	...
	def refresh_cursor(last_index,current_index)
		...
		@savefile_windows[current_index].selected = true
		@overview_window.set_file_index(current_index)
	end
  #--------------------------------------------------------------------------
  # * Move Cursor Down
  #--------------------------------------------------------------------------

...
class Window_Base < Window
  ...
  def update_tone
    ...
    self.tone.set(*System_Settings::MESSAGE_WINDOW_TONE)
  end
  #--------------------------------------------------------------------------
  ...
  #end
  def convert_escape_characters(text)
	  # First, inject external text from $game_text for \T[...] sequences
	  result = text.to_s.gsub(/\\T\[([^\]]+)\]/i) { $game_text[$1] }

	  # Then process standard escape sequences
	  result = result.clone
	  result.gsub!(/\\/)            { "\e" }
	  result.gsub!(/\e\e/)          { "\\" }
	  result.gsub!(/\eV\[(\d+)\]/i) { $game_variables[$1.to_i] }
  ...
  #--------------------------------------------------------------------------
	...
	# Process a single character with word wrap, slightly smaller to prevent overflow
	def process_normal_character_wordwarp(c, pos, text = "")
		# Measure character width
		text_width = (c == "\f") ? 0 : self.contents.text_size(c).width

		#line limit
		line_limit = self.contents.width + 4  # tweak as needed, smaller than before

		# Check if next word fits in the window
		if pos[:x] + next_word_width(c, text, text_width) >= line_limit
			pos[:x] = new_line_x
			process_new_line(c, pos)
		end

		# Draw the character with smaller buffer
		self.contents.draw_text(pos[:x], pos[:y], text_width + 1, pos[:height], c)

		# Advance cursor
		pos[:x] += text_width
	end

	# Calculate width of the next word (up to next space)
	def next_word_width(c, text, word_width)
		return 0 if c == "\e"
		c = "aa" if @str2Lang
		...
		if @str2Lang
			...
			return word_width unless textLength
			word_width + self.contents.text_size(text[0, textLength]).width
		end
	end
 ...
 # end
  ...
  def draw_item_name(item, x, y, enabled = true, width = 172)
    ...
    draw_text(x + 24, y, width, line_height, $game_text[item.name])
  end
  #--------------------------------------------------------------------------

...
class Game_Interpreter
  include GIM_GO  #module for accessing Game_Objects
  include GIM_OM  #module for overmap
  include GIM_PRT #module for portrait
  include GIM_CHCG #module for chcg functions
  include GIM_OVC #module for checking over events
  include GIM_ADDON
  #--------------------------------------------------------------------------
  ...
  def run
    ...
    while @list[@index] do
		begin
			...
			TKG::ErrorLog.save()
			p "Error in event ID: #{@event_id} command ID: #{@index} Error: #{ex} Error log file saved."
			msgbox "Error in event ID: #{@event_id} command ID: #{@index} Error: #{ex} Error log file saved."
			@index += 1
		end
    end
    Fiber.yield
  ...
  #  send(method_name) if respond_to?(method_name)
	...
	def execute_command #Teravisor mod
		return p "execute_command on a deleted event"  if @event_id!=0 && !get_character(0)
		command = @list[@index]
		@params = command.parameters
		@indent = command.indent
		method_name = "command_#{command.code}"
	...
	#	rescue => ex
	#			TKG::ErrorLog.save()
	#			p "Error in event command #{command.code} Error: #{ex} Error log file saved."
	#			msgbox "Error in event command #{command.code} Error: #{ex} Error log file saved."
	#	end
	#end
  #--------------------------------------------------------------------------
  # * Command Skip
  ...
  # * Screen Flash alias
  #--------------------------------------------------------------------------
  def flash_screen(color,duration,wait_till_stop)
    screen.start_flash(color,duration)
    wait(duration) if wait_till_stop
  end
  #--------------------------------------------------------------------------
  # * Screen Shake
  #--------------------------------------------------------------------------
...
=begin
	...
	[
		[	
			...
			item_number=> ....,
			use standard price => 0:use standard ; not zero: use[3],
			specify price
		],
		[0,50,0],
		[0,49,??,444]
...
#  end
  ...
  def command_354
	...
	p "disposing @interpreter"
	$game_portraits.dispose_portraits
    Fiber.yield
  end
  #--------------------------------------------------------------------------
  # * Script
  #--------------------------------------------------------------------------
  #def command_355 #original
  ...
  #      ev= get_character(0)
	#	if ev != nil
	#		raise $!, "#{$!.message} \nProblem Eval Event Script \n name=>#{ev.name} \n id=>#{ev.id} x=#{ev.x} y=#{ev.y}", $!.backtrace
	#	else
	#		raise $!, "#{$!.message} \nProblem Eval Event Script \n Event Not Found", $!.backtrace
	#	end
	#end
  #end
  def command_355
    ...
    begin
        ...
        if ev != nil
			...
			@list = [] #will end this  event's execution
			p "Tried calling script from deleted event: #{ex} in eval( #{script} )" #idk when this happens so print instead of msgbox
			msgbox "Tried calling script from deleted event: #{ex} in eval( #{script} )" #idk when this happens so print instead of msgbox
        end
    end
  end

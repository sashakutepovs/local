...
class Game_Character < Game_CharacterBase
  ...
  ROUTE_SCRIPT            = 45            # Script1111111
  #--------------------------------------------------------------------------
  # * Public Instance Variables
  #--------------------------------------------------------------------------
		attr_reader		:move_route_forcing       # forced move route flag
		attr_reader		:move_succeed 
		attr_accessor	:balloon_frame_speed
		attr_accessor	:balloon_frame_wait
		attr_accessor 	:balloon_repeat_time
		attr_accessor 	:wait_count
		attr_accessor 	:combo_original_move_route
		attr_accessor 	:original_move_route
		attr_accessor 	:original_move_route_index
  ...
  def init_public_members
    super
	@balloon_frame_speed = 10
	@balloon_frame_wait = 10
  ...
  # * Force Move Route
  #--------------------------------------------------------------------------
	def set_force_move_route(move_route)
	...
	def force_move_route(move_route)# rgss VER
		memorize_move_route unless @original_move_route
		set_force_move_route(move_route)
	end
	def combo_force_move_route(move_route) #COMBO SKILL VER
		@combo_original_move_route = @move_route
		@combo_original_move_route_index = @move_route_index
		set_force_move_route(move_route)
	end
	def combo_force_move_route_reset
		@move_route = @combo_original_move_route if @combo_original_move_route
		@move_route_index = @combo_original_move_route_index if @combo_original_move_route_index
	end
	def combo_force_move_route_clearn
		@combo_original_move_route = nil
		@combo_original_move_route_index = nil
	end
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
	def update_routine_move
		if @wait_count > 0
			...
			@move_succeed = true
			command = @move_route.list[@move_route_index] if @move_route
			if command
  ...
  def process_move_command(command)
    ...
    when ROUTE_SCRIPT;            
		begin
		 ...
		 #連我也沒搞懂這邊在幹嘛，source: https://stackoverflow.com/questions/2823748/how-do-i-add-information-to-an-exception-message-in-ruby
		 raise $!, "#{$!} \n Problem eval Path Command \n ev.name=>#{@event.name}\n @event.id=>#{@event.id} X#{@event.x} Y#{@event.y}\n move_route_index=>#{@move_route_index} \n", $!.backtrace
		end
    end
  end
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
	...
	def turn_toward_character_get_dir(character)
		sx = distance_x_from(character.x)
		sy = distance_y_from(character.y)
		if sx.abs > sy.abs
			sx > 0 ? 4 : 6
	...
	def turn_reverse_character(character)
		sx = distance_x_from(character.x)
		sy = distance_y_from(character.y)
		if sx.abs > sy.abs
	...
	def turn_reverse
		return @direction = 4 if @direction == 6
		return @direction = 6 if @direction == 4
		return @direction = 2 if @direction == 8
		return @direction = 8 if @direction == 2
	end
	def facing_character?(character)
		tmpDir = @direction
		sx = distance_x_from(character.x)
		sy = distance_y_from(character.y)
		if sx.abs > sy.abs
			sx > 0 ? tmpDir=4 : tmpDir=6
		elsif sy != 0
			sy > 0 ? tmpDir=8 : tmpDir=2
		end
		tmpDir == @direction
	end
	def facing_xy?(tmpX,tmpY)
		tmpDir = @direction
		sx = distance_x_from(tmpX)
		sy = distance_y_from(tmpY)
		if sx.abs > sy.abs
			sx > 0 ? tmpDir=4 : tmpDir=6
		elsif sy != 0
			sy > 0 ? tmpDir=8 : tmpDir=2
	...
	def get_facing_xy
		...
		when 8 # Up
			...
			[@x, @y] # Unknown direction fallback (no movement)
		end
	end
	def turn_toward_xy(temp_x=self.x , temp_y=self.y)
		return turn_random if temp_x == self.x && temp_y == self.y
		sx = distance_x_from(temp_x)
		sy = distance_y_from(temp_y)
		if sx.abs > sy.abs
			set_direction(sx > 0 ? 4 : 6)
		elsif sy != 0
			set_direction(sy > 0 ? 8 : 2)
	...
	def get_turn_toward_dir(temp_x=self.x , temp_y=self.y)
		return turn_random if temp_x == self.x && temp_y == self.y
		sx = distance_x_from(temp_x)
		sy = distance_y_from(temp_y)
		if sx.abs > sy.abs
	...
	def faced_character?(character)
		...
		return false if self.direction != temp_dir
		return true if self.direction == temp_dir
	end
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
	def turn_away_from_character(character)
		sx = distance_x_from(character.x)
		sy = distance_y_from(character.y)
		if character.x == self.x && character.y == self.y
			turn_random
		elsif sx.abs > sy.abs
			set_direction(sx > 0 ? 6 : 4)
		elsif sy != 0
  ...
  def move_forward_force
    move_straight_force(@direction)
  end
  #--------------------------------------------------------------------------
  # * 1 Step Backward
  #--------------------------------------------------------------------------
  ...
  def jump(x_plus, y_plus,tmpPeak=10)
    ...
    @jump_peak = tmpPeak + distance - @move_speed
  ...
  def jump_low(x_plus, y_plus,tmpPeak=5)
    ...
    @y += y_plus
    distance = Math.sqrt(x_plus * x_plus + y_plus * y_plus).round
    @jump_peak = 5 + distance - @move_speed
    @jump_count = @jump_peak * 2
    @stop_count = 0
    straighten
  end
  ...
  def balloon_id=(id)
    @balloon_id = id
    @balloon_frame_speed = 4
    @balloon_frame_wait = 12
    @balloon_repeat_time = 0
  end
   
  def call_balloon(id,time=0) #  ; time 0單次  -1Loop >1LOOP次數
    @balloon_id = id
    @balloon_frame_speed = 4
    @balloon_frame_wait = 12
    @balloon_repeat_time =time
  end
  ...
  def behind_me?(target)
	case @direction
		when 2;return target.y < @y;
		when 4;return target.x > @x;
		when 6;return target.x < @x;
		when 8;return target.y > @y;
	end
  end
  
  #計算target在自身的哪個方向
	def relative_dir?(target)

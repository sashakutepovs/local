...
#===============================================================================
	...
	#--------------------------------------------------------------------------
	UP    = [KEYMAP[DataManager.get_text_constant("Keyboard","UP",20).intern]]
	DOWN  = [KEYMAP[DataManager.get_text_constant("Keyboard","DOWN",20).intern]]
	LEFT  = [KEYMAP[DataManager.get_text_constant("Keyboard","LEFT",20).intern]]
	RIGHT = [KEYMAP[DataManager.get_text_constant("Keyboard","RIGHT",20).intern]]
	B     = [KEYMAP[DataManager.get_text_constant("Keyboard","B",20).intern],KEYMAP[DataManager.get_text_constant("Keyboard","X_LINK",20).intern]]
	C     = [KEYMAP[DataManager.get_text_constant("Keyboard","C",20).intern],KEYMAP[DataManager.get_text_constant("Keyboard","Z_LINK",20).intern]]
	L     = [KEYMAP[DataManager.get_text_constant("Keyboard","L",20).intern]]
	R     = [KEYMAP[DataManager.get_text_constant("Keyboard","R",20).intern]]
	F5    = [KEYMAP[:F5]]
	F6    = [KEYMAP[:F6]]
	F7    = [KEYMAP[:F7]]
	F8    = [KEYMAP[:F8]]
	F9    = [KEYMAP[:F9]]
	SHIFT = [KEYMAP[DataManager.get_text_constant("Keyboard","SHIFT",20).intern]]
	CTRL  = [KEYMAP[DataManager.get_text_constant("Keyboard","CTRL",20).intern]]
	ALT   = [KEYMAP[DataManager.get_text_constant("Keyboard","ALT",20).intern]]
	S1    = [KEYMAP[DataManager.get_text_constant("Keyboard","S1",20).intern]]
	S2    = [KEYMAP[DataManager.get_text_constant("Keyboard","S2",20).intern]]
	S3    = [KEYMAP[DataManager.get_text_constant("Keyboard","S3",20).intern]]
	S4    = [KEYMAP[DataManager.get_text_constant("Keyboard","S4",20).intern]]
	S5    = [KEYMAP[DataManager.get_text_constant("Keyboard","S5",20).intern]]
	S6    = [KEYMAP[DataManager.get_text_constant("Keyboard","S6",20).intern]]
	S7    = [KEYMAP[DataManager.get_text_constant("Keyboard","S7",20).intern]]
	S8    = [KEYMAP[DataManager.get_text_constant("Keyboard","S8",20).intern]]
	S9    = [KEYMAP[DataManager.get_text_constant("Keyboard","S9",20).intern]]
	D8    = [KEYMAP[DataManager.get_text_constant("Keyboard","D8",20).intern]]
	D2    = [KEYMAP[DataManager.get_text_constant("Keyboard","D2",20).intern]]
	D4    = [KEYMAP[DataManager.get_text_constant("Keyboard","D4",20).intern]]
	D6    = [KEYMAP[DataManager.get_text_constant("Keyboard","D6",20).intern]]
...
#===============================================================================
	...
	###################### CLEAR all key. by Teravisor
	SetKeyboardState = Win32API.new("user32.dll", "SetKeyboardState",  "I", "I")
	def self.clear_keyboard_state
		0.upto(255) do |key|
			@state[key]=0
		end
		SetKeyboardState.call(@state.to_i)
	end
	#--------------------------------------------------------------------------
	...
	def self.press?(keys)
		if keys.is_a?(Numeric)
			...
			if SYM_KEYS.key?(keys)
				return SYM_KEYS[keys].any? {|key| (key != 0 and @pressed[key]  and !@triggered[key]) } #fix by Teravisor
				#return SYM_KEYS[keys].any? {|key| (@pressed[key]  and !@triggered[key]) } #original
	...
	def self.trigger?(keys)
		if keys.is_a?(Numeric)
			...
			if SYM_KEYS.key?(keys)
				...
				return SYM_KEYS[keys].any? {|key| (key!=0 and @triggered[key])}
			elsif KEYMAP.key?(keys)
	...
	def self.repeat?(keys)
		if keys.is_a?(Numeric)
			...
			if SYM_KEYS.key?(keys)
				...
				return SYM_KEYS[keys].any? {|key| key!=0 and @repeated[key] == 1 || @repeated[key] == 16}
			elsif KEYMAP.key?(keys)
	...
	def self.release?(keys)
		if keys.is_a?(Numeric)
			...
			if SYM_KEYS.key?(keys)
				...
				return SYM_KEYS[keys].any? {|key| key!=0 and @released[key]}
			elsif KEYMAP.key?(keys)

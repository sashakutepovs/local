...
class Bitmap
	...
	def last_row_address
		#1. creates buf array and turns it into binary to get raw data from RAM
		#2. it gets id*2 of Bitmap which is its spot in memory plus 16 (that's 4 ints), probably some reference is stored there
		#3. it moves into that reference and gets a new link's address+8 (+2 ints). Probably another reference in there.
		#4. it moves into that reference+16 (+4 ints). Probably another reference.
		#5. And returns it.
	...
	def get_data_ptr
		data = String.new
		RtlMoveMemory.call(data.__id__*2, [0x6005].pack('L'), 4)
...
module Graphics
  ...
  class << self
    ...
    def width()  Graphics.width end unless method_defined?(:width)
    def height() Graphics.height end unless method_defined?(:height)

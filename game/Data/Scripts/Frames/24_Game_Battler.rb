...
class Game_Battler < Game_BattlerBase
  ...
  #--------------------------------------------------------------------------
	def reset_state_counts(state_id)
		...
		return prp "remove_state #{state_id} not found",1 if !state_id
		...
		@state_frames[state_id] = state.steps_to_remove * 60
	end
  #--------------------------------------------------------------------------
  # * Remove State
  #--------------------------------------------------------------------------
 ...
 #  user.mp += @result.mp_drain
  ...
  #--------------------------------------------------------------------------
	def use_item(item)
		...
		consume_item(item)   if item.consumable
	end
  #--------------------------------------------------------------------------
  # * Consume Items
  #--------------------------------------------------------------------------
...
#	#  !(state_stack(effect.data_id) >= $data_states[effect.data_id].max_stacks)
 ...
 #  self.tp += value
  ...
  #--------------------------------------------------------------------------
	def state_addable?(state_id)
		...
		alive? && $data_states[state_id] && !state_restrict?(state_id)
	end
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
	def add_new_state(state_id)
		...
		@states.unshift(state_id)
		on_restrict if restriction > 0
		sort_states
		refresh
	end
  # overwrite: states are sorted by their name color.
  def sort_states
    #sort_order = [:green, :yellow, :red, :magenta]
    #@states = @states.sort_by{|i| sort_order.index($data_states[i].color) || 999}
  end
end

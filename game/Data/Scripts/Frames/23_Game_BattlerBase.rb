...
#	end
  ...
  #--------------------------------------------------------------------------
  #--------------------------------------------------------------------------
  # * Determine if Equipment Is Locked Sealed
  #--------------------------------------------------------------------------
	def equip_slot_fixed_hard?(etype_id)
		etype_id = $data_system.equip_type_name[etype_id] if etype_id.is_a?(String)
		return true if fixed_hard_etypes.include?(etype_id) if actor?
		return false
	end
	def equip_slot_fixed?(etype_id)
		etype_id = $data_system.equip_type_name[etype_id] if etype_id.is_a?(String)
		return true if fixed_etypes.include?(etype_id) if actor?
		return false
		#features_set(FEATURE_EQUIP_FIX).include?(etype_id)
	end
	def equip_slot_sealed?(etype_id)
		etype_id = $data_system.equip_type_name[etype_id] if etype_id.is_a?(String)
	...
	def wtype_sealed?(wtype_id)
		feature_objects.uniq.each { |o| return true if o.wtype_seal.include?(wtype_id) }
		return false
	end
	def atype_sealed?(atype_id)
		feature_objects.uniq.each { |o| return true if o.atype_seal.include?(atype_id) }
		return false
	end
	def equip_wtype_ok?(wtype_id)
		feature_objects.uniq.each { |o| return true if o.wtype_okay(wtype_id) }
		return false
		#features_set(FEATURE_EQUIP_WTYPE).include?(wtype_id)
	end
	
	def equip_atype_ok?(atype_id)
		feature_objects.uniq.each { |o| return true if o.atype_okay(atype_id) }
		return false
	...
	def added_skills
		...
		feature_objects.uniq.each { |o|
			tmpAry = tmpAry | o.added_skills
		}
		tmpAry
		#features_set(FEATURE_SKILL_ADD)
	end
	def skill_sealed?(skill_id)
		return feature_objects.uniq.any?{|o| o.skill_sealed.include?(skill_id)}
		#features_set(FEATURE_SKILL_SEAL).include?(skill_id)
	end
	def equippable?(item)
		return false unless equippable_lv1_chk?(item)
		return false if item.is_a?(RPG::Weapon) && wtype_sealed?(item.wtype_id)
		return false if item.is_a?(RPG::Armor) && atype_sealed?(item.atype_id)
		return true
	end
	
	def equippable_lv1_chk?(item)
		return false unless item && item.etype_id
		return false if equip_slot_sealed?(item.etype_id)
		return equip_wtype_ok?(item.wtype_id) if item.wtype_id
		return equip_atype_ok?(item.atype_id) if item.atype_id
		return false
	end

	def common_equippable?(item)
		return false if !equippable?(item)
		return false if equip_slot_sealed?(item.etype_id)
		return false if equip_slot_fixed?(item.etype_id)
		true
	end
	def common_unequippable?(slot)
		slot = $data_system.equip_type_name[slot] if slot.is_a?(String)
		return false if equip_slot_sealed?(slot)
		return false if equip_slot_fixed?(slot)
		true
	end

	#will report [equip_slot] if its sealed or fixed ur item. EQUIP SLOT ONLY
	#when return [] mean fully equippable?
	#when ![].empty? array mean blocked by something
	#when false  mean not possible cant equip
	def report_reverse_equippable(item)
		return false if item.etype_id.nil?
		tgt_slot = item.etype_id
		finalReport = []
		feature_objects.uniq.each {|tmp_F_OBJ|
			next unless tmp_F_OBJ
			next if tmp_F_OBJ.is_a?(RPG::Actor)
			next if tmp_F_OBJ.is_a?(RPG::Class)
			next unless !tmp_F_OBJ.fixed_equip_type.empty? || !tmp_F_OBJ.sealed_equip_type.empty? || !tmp_F_OBJ.wtype_seal.empty? || !tmp_F_OBJ.atype_seal.empty?
			next unless tmp_F_OBJ.fixed_equip_type.include?(tgt_slot) || tmp_F_OBJ.sealed_equip_type.include?(tgt_slot) || tmp_F_OBJ.wtype_seal.include?(item.wtype_id) || tmp_F_OBJ.atype_seal.include?(item.atype_id)
			if tmp_F_OBJ.is_a?(RPG::State)
				return false if tmp_F_OBJ.fixed_equip_type.include?(tgt_slot)
				return false if tmp_F_OBJ.sealed_equip_type.include?(tgt_slot)
				return false if tmp_F_OBJ.wtype_seal.include?(item.wtype_id)
				return false if tmp_F_OBJ.atype_seal.include?(item.atype_id)
			end
			next unless tmp_F_OBJ.etype_id
			#next unless common_unequippable?(tmp_F_OBJ.etype_id)
			return false if !common_unequippable?(tmp_F_OBJ.etype_id)
			finalReport << tmp_F_OBJ.etype_id
		}
		finalReport << tgt_slot if self.equips[tgt_slot] # insert self is theres
		return false if !finalReport
		finalReport.uniq
	end
	def report_reverse_unequip_by(item)
		finalReport = []
		item.sealed_equip_type.each{|slot|
			next unless self.equips[slot]
			finalReport << slot
		}
		item.wtype_seal.each{|slot|
			next unless self.equips[slot] && item.wtype_seal.include?(self.equips[slot].wtype_id)
			finalReport << slot
		}
		item.atype_seal.each{|slot|
			next unless self.equips[slot] && item.atype_seal.include?(self.equips[slot].atype_id)
			finalReport << slot
		}
		finalReport.uniq
	end
	...
	def effect_allowed?(le)
		if !le.stats_key.nil?
			if le.stats_filter
				return le.stats_value.include?(self.stat[le.stats_key])
			else
				return !le.stats_value.include?(self.stat[le.stats_key])
			end
		end
 ...
 #  $game_party.in_battle ? item.battle_ok? : item.menu_ok?
  ...
  #--------------------------------------------------------------------------
	def usable?(item)
		...
		return false if !item.consumable
		return true if item.consumable
		return item_conditions_met?(item)  if item.is_a?(RPG::Item)
		#return item_conditions_met?(item)  if item.is_a?(RPG::Armor)
		#return item_conditions_met?(item)  if item.is_a?(RPG::Weapon)
		return false
	end
  #--------------------------------------------------------------------------

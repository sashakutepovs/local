...
module YEA
  module MESSAGE
    ...
    #TEXT_SKIP = :CTRL    # Input::A is the shift button on keyboard.
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    NAME_WINDOW_X_BUFFER = 10     # Buffer x position of the name window.
...
module Variable
  ...
  def self.message_rows
    ...
    return 4 if System_Settings::MESSAGE_WINDOW_VISIBLE_LINES <= 0
    #return $game_variables[YEA::MESSAGE::VARIABLE_ROWS]
	return System_Settings::MESSAGE_WINDOW_VISIBLE_LINES
  end
  ...
  def self.message_width
   ...
   # return Graphics.width if $game_variables[YEA::MESSAGE::VARIABLE_WIDTH] <= 0
    ...
    return Graphics.width if System_Settings::MESSAGE_WINDOW_WIDTH <= 0
	case $game_message.background
	when 0;	return System_Settings::MESSAGE_WINDOW_WIDTH
	when 1;	return System_Settings::MESSAGE_WINDOW_WIDTH_DIM
	when 2;	System_Settings::MESSAGE_WINDOW_WIDTH
	else
	return System_Settings::MESSAGE_WINDOW_WIDTH
	end
  end
...
class Window_Base < Window
  ...
  def setup_message_font
    ...
    contents.font.size = System_Settings::FONT_SIZE::MESSAGE_WINDOW
    contents.font.bold = System_Settings::MESSAGE_WINDOW_FONT_BOLD
    contents.font.italic = System_Settings::MESSAGE_WINDOW_FONT_ITALIC
    contents.font.outline = System_Settings::MESSAGE_WINDOW_FONT_OUTLINE
    contents.font.shadow = System_Settings::MESSAGE_WINDOW_FONT_SHADOW
  end
...
class Window_Message < Window_Base
  ...
  def update_show_fast
    @show_fast = true if Input.MsgSkipKeyPressed?
  ...
  def convert_escape_characters(text)
	...
	#make #{} works
	alias_method :convert_escape_characters_eval, :convert_escape_characters
	def convert_escape_characters(text)
		result = convert_escape_characters_eval(text)
		result = find_and_eval_interpolations(result)
		return result
	end
		...
		while i < a_string.length-1
			...
			if c== "}" && left_bracket>0 then
				...
				if left_bracket == 0 then
					return a_string[0...start_char]+eval('"'+a_string[start_char..i]+'"')+find_and_eval_interpolations(a_string[i+1...a_string.length])
				end
			end
			i += 1
		end
...
#==============================================================================
# ** SOUND PLAY WHEN DIALOG    added 2018 12 24
#==============================================================================
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
	...
	def process_normal_character(c, pos, text)
		if $game_message.position!=3 && !$game_message.narrator
			tse_process_normal_character(c, pos, text)
			!@dialogSndVol ? tmpSndVol = rand(20)+50 : tmpSndVol = rand(20)+@dialogSndVol
			!@dialogSndPitch ? tmpSndPitch= 60+rand(20) : tmpSndPitch= @dialogSndPitch+rand(20)
			!@dialogSndDense ? tmpSndDense = System_Settings::MESSAGE_SOUND_CLOSENESS : tmpSndDense = @dialogSndDense
			!@dialogSndFile ? tmpSndFile = "Cursor2" : tmpSndFile = @dialogSndFile
			if @show_fast
				SndLib.sys_Dialog(tmpSndVol,tmpSndPitch,tmpSndFile) if @character % System_Settings::MESSAGE_SOUND_CLOSENESS_SHOWFAST == 0
			else
				SndLib.sys_Dialog(tmpSndVol,tmpSndPitch,tmpSndFile) if @character % tmpSndDense == 0
			end
			@character += 1
		elsif $game_message.narrator
			tse_process_normal_character(c, pos, text)
			@character_pos = pos[:x] if pos[:x] > @character_pos #置中
			self.width =  @character_pos+30
	...
	def setdialogSndPitch(tmpTar)
		@dialogSndPitch = tmpTar
	end
	def setdialogSndDense(tmpTar)
		@dialogSndDense = tmpTar
	end
	def setdialogSndVol(tmpTar)
		@dialogSndVol = tmpTar
	end
	def setdialogSndFile(tmpTar)
		@dialogSndFile = tmpTar
	end
  
  #集中處理@character_pos的問題
	alias ks_centered_process_narrator_start process_narrator_start
	def process_narrator_start
			@character_pos=0
			ks_centered_process_narrator_start
	end
  #--------------------------------------------------------------------------

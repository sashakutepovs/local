...
#哥布林AI，走三部打一步
class Npc_HitRunBackAttack < Game_NonPlayerCharacter
	...
	def get_move_command
		...
		case @event.npc.ai_state
			when :assaulter,:fucker
				...
				return [:turn_reverse_character,tgt] if @event.near_the_target?(tgt,2) && tmpWayBlocked >= 4 # if no where to run, do melee, remove tmpWayBlocked if he is good at melee

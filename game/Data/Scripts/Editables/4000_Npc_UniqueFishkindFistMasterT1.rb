...
class Npc_UniqueFishkindFistMasterT1 < Game_NonPlayerCharacter
	...
	def setup_npc
		...
		@stance_default_list = [
						...
						#text
		@skill_Forward		= ["FishFistMasterBasicCombo","FishFistMasterTatsumakiHkHold"]
	...
	def update_step_sndlib
		...
		case @event.pattern
			...
			when 2,0
				SndLib.sound_HeavyStep(@event.report_distance_to_vol_close-10) if @stepSndLibPlayable == true
				@stepSndLibPlayable = false
		end
	end
	...
	def set_killer_skill(dist)
		...
		play_sound(:sound_skill,map_token.report_distance_to_vol_close_npc_vol)
	end
	...
	def apply_stun_effect(state_id)
		msgbox @stun_cover_count
		@stun_cover_count += 1
		return if @stun_cover_count < 3
		@stun_cover_count = 0
		super("Stun1")
	end

...
#標準型人類，發現目標就是衝過去狂扁
#this RB is a copy paste from character like GrayRat.   for temp use
class Npc_UniquePigBobo < Game_NonPlayerCharacter
	def update_frame
	...
	def get_move_command
		...
		tgt = get_target
		@event.npc.process_target_lost if !tgt.nil? && (@event.actor.target == master || @event.actor.fraction == tgt.actor.fraction) #tgt.npc.npc_name == "UniqueCecily"

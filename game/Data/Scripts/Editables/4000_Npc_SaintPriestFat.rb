...
class Npc_SaintPriestFat < Game_NonPlayerCharacter
	...
	def stance_next
		...
		@stance_wait			= @stance_current[2]
		case @stance_name
			when :Attack			;skills = ["BasicSaintJudgment","NpcCurvedSaintSmite", "NpcOrcPunchHeavyMH","NpcOrcPunchHeavyChain","NpcMarkMoralityDown"]
									@skills_killer_list=@skills_assaulter_list=skills
									
			when :Support			;skills = ["NpcOrcPunchHeavyMH","NpcOrcPunchHeavyChain","NpcMarkMoralityDown"]
									@skills_killer_list=@skills_assaulter_list=skills
		end
		clear_cached_skill
	end

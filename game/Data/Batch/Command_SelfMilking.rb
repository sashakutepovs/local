...
$game_actors[1].mood -= (rand(2)+1) if $game_player.actor.stat["IronWill"] ==1 #change to -mood
#Global.good_happen  if aggrement sex
$game_actors[1].mood += (rand(2)+2) if $game_player.actor.stat["Nymph"] ==1 #change to +mood
$game_actors[1].mood += (rand(2)+2) if $game_player.actor.stat["Masochist"] ==1 #change to +mood
...
$game_actors[1].arousal += rand(3) * ($game_player.actor.stat["AsVulva_Skin"]*10) #皮膚性器化
$game_actors[1].arousal += 40*(10*$game_player.actor.stat["AsVulva_MilkGland"]+$game_actors[1].sensitivity_breast) #B感度+B性器化
...
$game_actors[1].arousal += 5 if $game_player.actor.stat["FeelsWarm"] ==1 # feels warm

...
$game_actors[1].mood -= (rand(5)+10) if $game_player.actor.stat["Prostitute"] ==0 #change to -mood
$game_actors[1].mood -= (rand(8)+20) if $game_player.actor.stat["IronWill"] ==1 #change to -mood
#Global.good_happen  if aggrement sex
$game_actors[1].mood += (rand(5)+19) if $game_player.actor.stat["Masochist"] ==1 #change to +mood
tmpSight = true
#------------------------------------------------------------- #每FRAME EXP 接收
tmpBouns = 1
...
$game_actors[1].arousal += rand(100) if $story_stats["sex_record_vag_dilatation"] >=15
$game_actors[1].arousal += rand(100) if $game_player.actor.stat["Masochist"] ==1
$game_actors[1].vag_damage += rand(700)
$game_actors[1].urinary_level += rand(150)
$game_actors[1].health -= 3+rand(4) if $story_stats["sex_record_vag_dilatation"].between?(0,14)&&$game_actors[1].state_stack(41) < 1
$game_actors[1].sta -= 2+rand(3)
#$game_actors[1].sta += 3+rand(2) if $story_stats["sex_record_vag_dilatation"].between?(0,14)&&$game_actors[1].state_stack(41) < 1
$game_actors[1].baby_health -= 5+rand(3) if $game_actors[1].preg_level !=0



$game_actors[1].health -=  3+rand(3) if $story_stats["Setup_Hardcore"] >= 1
$game_actors[1].baby_health -= 5+rand(3) if $game_actors[1].preg_level !=0 && $story_stats["Setup_Hardcore"] >= 1

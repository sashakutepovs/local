...
#-------------------------------------------------------------
 $game_player.actor.arousal += rand(25) if $game_player.actor.stat["Nymph"] ==1
 $game_player.actor.arousal += rand(35) if $story_stats["sex_record_urinary_incontinence"]>= 15
 $game_player.actor.arousal += rand(60) if  $game_player.actor.state_stack(105) !=0
 $game_player.actor.arousal += rand(60) if $game_player.actor.stat["AsVulva_Urethra"] ==1
 $game_player.actor.sta -= 1
 $game_player.actor.dirt += rand(2)+2
 $game_player.actor.dirt += rand(2)+3 if $game_map.interpreter.IsChcg?
$game_map.reserve_summon_event("WastePee") if !$game_map.isOverMap && rand(100)>=50
# $game_player.actor.mp += rand(3) if masturbation

SndLib.sound_chcg_pee(100,200)
